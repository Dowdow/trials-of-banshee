...
export default function Triumph({ type, title, description, badge = false, claimable = false, completed = false }) {
  ...
  return (
    <button type="button" disabled={!claimable || completed} onClick={handleClick} className={`relative bg-white/10 border ${claimable && 'border-green-light'} ${completed && 'border-yellow'} ${!claimable && !completed && 'border-white/30'}`}>
      {claim && (
        <>
